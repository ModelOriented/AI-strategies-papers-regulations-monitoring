...
def score_embeddings_for_documents(
    ...
    if emb_type is None:
        ...
        return
    todo_docs = mars.db.get_ids_of_docs_between(key_min, key_max)
    logger.info("All docs: %s", len(todo_docs))
    for doc_key in todo_docs:
        for sents_docs in fetch_batches_until_empty(
            f'FOR u IN {collections.SENTENCES} FILTER u.{db_fields.SENTENCE_DOC_ID} == "{doc_key}" && (u.{db_fields.EMBEDDING}.{emb_type} == NULL || u.{db_fields.EMBEDDING}.{emb_type} == NULL) LIMIT 100 RETURN u'
        ):
            logger.info(
            ...
            for embedding, sent_doc in zip(embeddings, sents_docs):
                ...
                sent_doc[db_fields.EMBEDDING][emb_type] = (
                    embedding if type(embedding) == np.ndarray else embedding.numpy()
                ).tolist()
                sent_doc.forceSave()
