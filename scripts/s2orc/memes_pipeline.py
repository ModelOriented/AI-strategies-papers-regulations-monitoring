...
def pipeline(
    ...
    if do_cluster:  #not os.path.exists(out_path_cluster):
        ...
        chunk_to_embedding = embedd_noun_chunks.embedd_noun_chunks(
            in_path, sentences_embedding, batch_size_embedd,
            multiprocess_embedd)  #return chunk to embeddinng

        #reduce dimensionality
        if do_reduce:
            print('REDUCING')
            reduced = reduce_dimensionality.reducing(chunk_to_embedding,
                                                     n_components_reduce,
                                                     gpu_reduce)
        else:
            reduced = chunk_to_embedding

        pd.DataFrame(chunk_to_embedding).to_parquet(
            out_path_embedding)  #saving

        print('CLUSTERING')
        df_cluster = cluster.clustering(reduced, n_jobs_cluster,
                                        cluster_selection_epsilons,
                                        gpu_cluster, min_clust_size,
                                        metric_cluster)
    ...
    if do_memes:
        if not os.path.exists(aff_output_path):
            ...
            df_af = affiliation_pipeline.affiliations(condition_list, category,
                                                      cit_path, json_input,
                                                      aff_output_path)
        else:
